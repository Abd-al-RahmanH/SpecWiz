...
class PrepareVectorDB:
    ...
    def append_to_vector_db(self):
        doc_directory = self.data_directory
        doc = PyPDFLoader(doc_directory[0]).load()
        print("Number of pages:", len(doc), "\n\n")
        chunked_document = self.__chunk_documents(doc)
        vectordb = Chroma(
            persist_directory=self.persist_directory, embedding_function=self.embedding
        )
        vectordb.add_documents(documents=chunked_document, embedding=self.embedding)
